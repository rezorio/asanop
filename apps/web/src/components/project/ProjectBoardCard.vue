...
const dueClass = computed(() => {
  if (urgency.value === 'overdue') {
    return isMine.value ? 'text-[#fecaca] font-semibold' : 'text-danger'
  }
  if (urgency.value === 'today') {
    return isMine.value ? 'text-white font-semibold' : 'text-charcoal font-semibold'
  }
  return isMine.value ? 'text-white/75' : 'text-muted'
})
  ...
  >
    <div class="mb-1.5 flex items-start justify-between gap-2">
      <p
        ...
        :class="[
          isMine ? 'text-white' : 'text-charcoal',
          isDone ? (isMine ? 'line-through decoration-white/50' : 'line-through decoration-charcoal/70') : '',
        ]"
      >
        {{ task.title }}
      </p>
      <span
        ...
        :class="isMine ? 'bg-white/20 text-white' : 'bg-brand-soft text-brand'"
      ...
      <span
        v-else
        class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-dashed text-[8px] font-semibold"
        :class="isMine ? 'border-white/40 text-white/80' : 'border-brand/40 text-brand/70'"
        title="Unassigned"
      >
        ?
      </span>
    ...
    <div v-if="hasMeta" class="flex flex-wrap items-center gap-x-1.5 gap-y-1">
      <span
        ...
        :class="isMine ? 'text-white/70' : 'text-muted'"
      >
        {{ task.section.name }}
      </span>
      ...
      <span
        v-if="task.isBlocked"
        class="inline-flex items-center gap-0.5 text-[10px] font-medium"
        :class="isMine ? 'text-[#fecaca]' : 'text-danger'"
      >
